 ...
 /**
	...
	*/
	function check_webdav() {
        ...
        var_dump($resp);
        //if ( ! array_key_exists( 'DAV', $resp['header']) ) $resp['header']['DAV'] = '1,2';
	...
	function _process_respond() {
		...
		for($i=1; $i<count($lines); $i++) {
			...
			if (!$header_done ) {
                ...
                if (!array_key_exists('header', $ret_struct)) $ret_struct['header'] = Array();
				if ( ! array_key_exists( $fieldname, $ret_struct['header'] ) ) {
